    ...
    <div className="bg-white p-4 w-full">
      <div className="flex justify-center mb-4 w-full">
        <input
          ...
          className="px-4 py-2 rounded-lg border border-gray-300 bg-gray-200 w-full sm:w-2/3 lg:w-1/2"
        />
      </div>
      <div className="w-full overflow-x-auto scroll-smooth">
        <div className="flex justify-start space-x-4 text-gray-900 flex-nowrap py-2 px-4">
          {categories.map(category => (
            <span
              key={category}
              className={`rounded-full px-6 py-2 cursor-pointer whitespace-nowrap
                 ${category === selectedCategory ? "text-white" : "text-gray-900"} 
              ${category === selectedCategory ? "bg-gray-900" : "bg-transparent"}
